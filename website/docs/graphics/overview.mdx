...
@use "glsass" with ($g-color-black: #111);
@use "glsass/graphics" with(
  $ui-form-background-color: cv(black), // ✓ correct, will equal to `#111`
);

// Alternative setups
@use "glsass" with ($g-color-black: #111) as *; // avoid namespace
@use "glsass/graphics" with(
  $ui-form-background-color: glsass.cv(black), // ✓ aslo correct!
);
```

:::note
Inside the codebase, graphics are also called `ui` to simplify the namespace.
:::
