...
The [cv() function](tools/concept.mdx#cv) offer a simpler way to access to a
specific color instead of use the variable name like `$g-color-primary` or
`$g-color-gray-800`.
...
[yiq-contrast() function](tools/concept.mdx#yiq-contrast).
