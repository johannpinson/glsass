...
for the from components (checkbox, radio, select, validation states), but it
can be extended to use it in your own code
...
The `$g-icons-database` need a map with some default configurations to work
with the internal _API_ and to be available use through the `iv()` helper
function.
...
The `options` map allow to replace custom string from the inital SVG string to
a "dynamic" value.<br/>
It can be usefull to use the same icon, but with different colors depeding on
a specific state (hover, focus, etc.).
