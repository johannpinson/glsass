...
One of the first tools is `Normalize.css` to homogenize some behaviors
**accross browsers**. Then, with the Meyer reset, it removes default styling
from browser (on top of Normalize). Next, like the Reboot from Bootstrap, we
had some opinionated rules to prepare some elements to work with the rest of
the framework.

### Reset

Combining some rules from Normalize.css and the Meyer's reset, it improve
default renders for some elements.<br/>
For example, it removes extra margin and padding on sime elements and nested lists.
